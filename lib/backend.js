...
const { omitUnderscored } = require('./utils')
...
exports.Backend = class Backend {
  ...
  member(query) {
    if (typeof query === 'object' && query != null) {
      /*
        TODO: Swtich to `user.profile.display_name` once `display_name` is
    ...
    return this.table('Members').then(members => {
      return findInArray(members, query, fields, 'member')
    })
  }
  ...
  delete(table, id) {
    ...
    return new Promise((resolve, reject) => {
      this.base(table).destroy(id, (err, res) => {
        ...
        if (this.data[table]) {
          this.data[table] = this.data[table].filter(row => row._id !== id) || []
        }
  ...
  places() {
    const { results } = require('../places-results')
    results.forEach(r => r._type = 'GooglePlace')
...
exports.LIST_ARGS = {
  ...
  Sessions: {sort: [{field: 'Start'}], fields: 'Start Location Address GooglePlaceID'.split(' ')},
