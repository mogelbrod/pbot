...
export function startBot(cfg) {
  ...
  const eventHandlers = {
    ...
    message(ev) {
      ...
      if (ev.channel[0] !== 'D' && !mentionRegex.test(ev.text)) {
        return
      }

      // Remove @mention prefix and convert `<value|text>` into `text`
      const cleaned = ev.text
        ...
        .replace(
          ...
          (m, type, link, label) => {
            ...
            if (type === '@' || type === '#') {
              const entity = bot[type === '@' ? 'users' : 'channels'].find(
                (x) => x.id === link,
              )
      ...
      if (cleaned === 'reload') {
        log('Received reload request - refreshing cached users')
        promise = bot.getUsers().then((result) => {
          log(`Reloaded ${result.members.length} users`)
          users = result.members
        })
      }
      ...
      return promise.then(() => {
        ...
        log(
          `Executing '${cleaned}' triggered by ${(user && user.name) || ev.user}`,
        )
    ...
    team_join(ev) {
      ...
      users.push(ev.user)
      log(
        `Adding newly joined user '${(ev.user && ev.user.name) || ev.user}' to users list`,
      )
    },
  }
  ...
  bot.on('message', (ev) => {
    ...
    if (handled) {
      ...
      Promise.resolve()
        .then(() => eventHandlers[ev.type](ev))
        .then((messageBody) => {
          return messageBody == null ? null : format.fancy(messageBody)
        })
        .catch((err) => {
        ...
        .then((messageBody) => {
          return messageBody == null ? null : message(ev.channel, messageBody)
        })
    }
  })
  ...
  function assertMessageReceived() {
    if (Date.now() - timeout > lastMessageTime) {
      log(
        `No messages received since ${lastMessageTime.toLocaleTimeString()}, triggering reconnect`,
      )
      closeAndReconnect()
    }
  }
  ...
  function message(target, message) {
    return bot
      .postMessage(target || cfg.defaultChannel, message, {
        icon_emoji: ':beers:',
        mrkdwn: true,
      })
