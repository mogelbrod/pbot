...
command('timer', 'Sets a timer (duration=cancel cancels most recent)', function(
  duration = '2h',
  message = 'Timer ended'
) {
  if (duration === 'cancel') {
  ...
  const timeout = setTimeout(() => {
    this.output([[ f.SlackVariable('!here'), `${message} (${duration})` ]])
  }, milliseconds)
...
command('suggest', 'Suggests establishments to visit', function(
  query = '',
  price = '0-4',
  openNow = 'no',
  results = 20,
  ...
  results = isInt(results) && results > 0 && results < 100 ? results : 20
  radius = isInt(radius) && radius > 0 ? radius : 5000
  let [minPrice, maxPrice] = price.split('-')
  if (!isInt(maxPrice)) {
    maxPrice = isInt(minPrice) && minPrice > 0 ? minPrice : '4'
    minPrice = '0'
  }
  openNow = !!openNow && openNow !== 'no'
...
command('maintenance-compress', 'Merges oldest drink sessions to free up the specified number of Airtable rows ', function(rows = 1) {
  rows = parseInt(rows, 10)
  if (!isInt(rows) || rows < 1 || rows > 1000) {
    rows = 1
  }
  let deleted = 0
  ...
  return Promise.all([
    ...
    allDrinks.forEach((drink) => {
      ...
      if (!memberId || !sessionId) {
        this.output(['Drink with empty session/member:', drink])
        return
      }
    ...
    for (let sessionId in grouped) {
      if (Object.prototype.hasOwnProperty.call(grouped, sessionId)) {
        ...
        for (let memberId in grouped[sessionId]) {
          if (Object.prototype.hasOwnProperty.call(grouped[sessionId], memberId)) {
            const drinks = grouped[sessionId][memberId]
            if (!drinks || drinks.length < 2) { continue }
            ...
            this.output(['Updating', aggregate])
            this.output(['Deleting'].concat(drinks.slice(1)))
            promises.push(
              // Update aggregate first, then delete old rows
              this.backend.updateRecord(aggregate).then(() => {
                return Promise.all(drinks.slice(1).map(drink => this.backend.deleteRecord(drink)))
              })
            )
          }
        }
        ...
        if (deletedInSession) {
          ...
          this.output([`Compressed ${deletedInSession} rows in`, session || sessionId])
        }
      }
    }
