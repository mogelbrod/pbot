...
command('suggest', 'Suggests establishments to visit', function(results = 5) {
  return Promise.all([
...
command('place', 'Displays info for a given Google place', function(query) {
  return this.backend.places().then(places => {
    return findInArray(places, query, ['name', 'vicinity'], {
      multiple: true,
      recordType: 'place',
    })
  })
...
command('maintenance', 'Various maintenance tasks', function() {
  ...
  return this.backend.table('Sessions').then(sessions => {
    ...
    const sessionUpdates = sessions.map(session => {
      ...
      const placePromise = session.GooglePlaceID
        ? Promise.resolve()
        : googlePlaces.findPlace(query, this.backend.config.googlePlacesKey)

      return placePromise.then(placeRes => {
        const place = placeRes && placeRes.candidates && placeRes.candidates[0]
        if (place) {
