...
command('sessions', 'Lists most recent/all sessions', function(limit = '20') {
...
command('maintenance-compress', 'Merges drinks in oldest sessions to free up Airtable rows', function(rows = 1) {
      ...
      const ranked = Object.keys(partitions)
        ...
        .sort((a, b) => {
          return forSession
            ...
            : new Date(b.Entity.Start) - new Date(a.Entity.Start)
        })
        .map((row, index) => [
          (index + 1) + '.',
          row.Entity,
...
function assertAdminUser(context) {
  ...
  return context.backend.member(context.user).then(member => {
