...
command("signup", "Registers a new member", function(email, name = "", role = "Member") {
  const memberPromise = this.user ? this.backend.member(this.user) : Promise.resolve()
  return memberPromise.then(member => {
    if (this.user && member && !name) {
      throw new Error(`${member.Name} already signed up`)
    }

    // Already signed up non-admin?
    if (this.user && !this.backend.isAdmin(member)) {
      if (role !== "Member") {
...
command("chuck-it", "I'll tell you a true Chuck Norris story", function() {
	
	function callChuckApi() {
	...
	return callChuckApi().then(function (res){
	    return ":muscle: " + res.value + ":muscle:"
	  })
})
