...
f.wrap = (delimeter, ...parts) => {
  if (typeof delimeter !== 'string' || !delimeter.length) {
    throw new TypeError(`wrap: Delimeter must be a non-empty string, got '${delimeter}'`)
  }
  const delimeterRegex = new RegExp(escapeRegExp(delimeter), 'g')
  const str = parts.join(' ').replace(delimeterRegex, '\\'+delimeter)
