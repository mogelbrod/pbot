...
f.fancy = (v, depth = 0) => {
  ...
  switch (type) {
    ...
    case "Date": return f.date(v, true)
    ...
    case "Sessions": return `${f.linkify(v.Location)} (${f.date(v.Start, true)})`
    case "Feedback": return `"${v.Feedback}" - ${f.fancy(v.Author, 2)} (${f.date(v._created, true)})`
    case "Quotes": return `"${v.Quote}" - ${f.fancy(v.Author, 2)} (${f.date(v._created, true)})`
